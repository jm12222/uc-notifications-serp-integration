    ...
    public var body: some View {
        HStack(spacing: 6) {
            if let selected = selectedReaction {
                ...
                Image("like-outline")
                    ...
                    .scaledToFit()
                    .frame(width: 20, height: 20)
                    .foregroundColor(onMedia ? Color("secondaryIconOnMedia") : Color("secondaryIcon"))
                    .modifier(ConditionalIconShadowModifier(onMedia: onMedia))
            }
            
            if count > 0 {
                Text(formatNumber(count))
                    .body4LinkTypography()
                    .foregroundColor(onMedia ? Color("secondaryTextOnMedia") : Color("secondaryText"))
                    .modifier(ConditionalTextShadowModifier(onMedia: onMedia))
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 8)
                .fill(onMedia ? Color("mediaPressed") : Color("nonMediaPressed"))
...
// MARK: - Conditional Shadow Modifiers

struct ConditionalTextShadowModifier: ViewModifier {
    let onMedia: Bool
...
struct ConditionalIconShadowModifier: ViewModifier {
    let onMedia: Bool
...
#Preview {
    VStack(spacing: 20) {
        ReactionPicker(count: 42) { reaction in
            print("Selected: \(reaction.id)")
        }
        
        Divider()
        
        ReactionPicker(count: 1234) { reaction in
            print("Selected: \(reaction.id)")
        }
