...
struct FDSUnitHeader: View {
    ...
    /// Determines if we should center-align the HStack (when there's only headline text with no body or meta)
    private var shouldCenterAlign: Bool {
        bodyText == nil && metaText == nil
    }
    
    // MARK: - Body
    var body: some View {
        VStack(spacing: 0) {
            HStack(alignment: shouldCenterAlign ? .center : .top, spacing: 12) {
            ...
            ScrollView {
                VStack(spacing: 12) {
                    ...
                    UnitHeaderGroupCard(title: "Right icon button") {
                        FDSUnitHeader(headlineText: "Personal details", hierarchyLevel: .level3, rightAddOn: .iconButton(icon: "pencil-outline", action: {}))
                    }
